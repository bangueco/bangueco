<h1 align="center">Hello, I am Ivan Gueco</h1>

👋 Hey there! I'm a passionate college student with a dream to become a proficient web/mobile developer. My journey revolves around the love for learning and exploring the endless possibilities of the digital world. My main focus is on JavaScript/Typescript, both for frontend and backend development, but I also have experience with a variety of other technologies.

Whether it's unraveling the intricacies of code or designing captivating user experiences, I thrive on the thrill of problem-solving and the satisfaction of seeing ideas come to life. Let's connect and embark on this exciting journey together! 🚀

# Current Tech Stack:
[![Current Tech Stack](https://skillicons.dev/icons?i=html,css,js,ts,tailwind,react,nodejs,express,postgres,sqlite,prisma,vitest,vite)](https://skillicons.dev)

# Dev Tools
[![Dev Tools](https://skillicons.dev/icons?i=arch,vscode,pnpm,postman,git,github,bash)](https://skillicons.dev)

# Other Technologies I Used
These are the technologies that i used in one my past projects giving me insights and familiarity
<br>
<br>
[![Other Tech](https://skillicons.dev/icons?i=next,php,laravel,python,cs,java,mysql,mongodb)](https://skillicons.dev)

# Currently Learning
I'm currently upskilling myself into by diving deep java, and learning springboot + docker and aws
<br>
<br>
[![Learning](https://skillicons.dev/icons?i=java,spring,docker,aws)](https://skillicons.dev)

# Github Stats
<p><img align="left" src="https://github-readme-stats.vercel.app/api/top-langs?username=bangueco&show_icons=true&locale=en&layout=donut&theme=onedark" alt="bangueco" /></p>
<p><img align="center" src="https://github-readme-streak-stats.herokuapp.com/?user=bangueco&theme=onedark" alt="bangueco" /></p>
